...
function getNumberOfRefinedLandmarks(refinements) {
    ...
    if (minIndex !== 0) {
        throw new Error("Indexes are expected to start with 0 instead of ".concat(minIndex));
    }
    if (maxIndex + 1 !== numIndices) {
        throw new Error("Indexes should have no gaps but ".concat(maxIndex - numIndices + 1, " indexes are missing"));
    }
    return numIndices;
}
...
function landmarksRefinement(allLandmarks, refinements) {
    ...
    for (var i = 0; i < allLandmarks.length; ++i) {
        ...
        if (landmarks.length !== refinement.indexesMapping.length) {
            ...
            throw new Error("There are ".concat(landmarks.length, " refinement landmarks while mapping has ").concat(refinement.indexesMapping.length));
        }
        // Refine X and Y.
